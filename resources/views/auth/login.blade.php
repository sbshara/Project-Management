  ...
  <div class="login-box-body">
    ...
    <form action="{{ route('login') }}" method="POST"  aria-label="{{ __('Login') }}">
      ...
      <div class="form-group has-feedback">
        <input type="email" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" required autofocus name="email" id="email" value="{{ old('email') }}" placeholder="{{ __('E-Mail Address') }}">
        @if ($errors->has('email'))
      ...
      <div class="row">
        <div class="col-xs-8">
          <div class="checkbox icheck">
            <label>
              <input type="checkbox" {{ old('remember') ? 'checked' : '' }} name="remember" id="remember" class="form-check-input iCheck"> {{ __('Remember Me') }}
            </label>
          </div>
        </div>
        <!-- /.col -->
